...
fn main() {
    ...
    let mut color_map: ColorMap = csv_read().expect("Erro ao ler o arquivo");
    ...
    csv_write(color_map).expect("Erro ao escrever o arquivo");
...
fn map_trained_colors(color_map: &ColorMap) -> Vec<TrainedMap> {
    ...
    let mut red_map: Vec<(usize, usize)> = vec![];
    let mut yellow_map: Vec<(usize, usize)> = vec![];
